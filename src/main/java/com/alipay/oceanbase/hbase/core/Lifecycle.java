...
public interface Lifecycle {
     ...
     * <em>strongly</em> encouraged to declare concrete implementations of the {@code init} method
     * to throw more specific exceptions, or to throw no exception at all if the initialize
     * operation cannot fail.
     *
     * <p>Cases where the initialize operation may fail require careful attention by implementers.
     * It is strongly advised to building the underlying resources and to internally <em>mark</em>
     * the resource as initialized, prior to throwing the exception. The {@code close} method is
     * unlikely to be invoked more than once and so this ensures that the resources are released in
     * a timely manner. Furthermore it reduces problems that could arise when the resource wraps, or
     ...
     * init} method should not throw it.
     *
     * <p>Note that this {@code init} method is <em>not</em> required to be idempotent. In other
     * words,calling this {@code close} method more than once may have some visible side effect.
     *
     * <p>However, implementers of this interface are strongly encouraged to make their {@code init}
     * methods idempotent.
     *
     ...
     *
     * <p>Note that unlike the {@link java.io.Closeable#close close} method of {@link
     * java.io.Closeable}, this {@code close} method is <em>not</em> required to be idempotent. In
     * other words, calling this {@code close} method more than once may have some visible side
     * effect, unlike {@code Closeable.close} which is required to have no effect if called more
     * than once.
     *
